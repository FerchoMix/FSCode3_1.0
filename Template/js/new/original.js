...
function agregarProducto() {
    ...
    if (miSelect.length > 0) {
        ...
        var stock = $('#comProStock' + ID).val(); // Asumiendo que tienes un campo para el stock
        var hoy = $('#hoydia').val();
        ...
        if (parseInt(stock) > 0) {
            ...
            <tr>
                ...
                <td><input id="detComCan${ID}" name="cantidad${ID}" onchange="calibrar(${ID})" type="number" min="1" max="${stock}" class="form-control" value=0></td>
...
function eliminarFila(ID) {
    opcion = confirm('¿Está seguro de eliminar el producto seleccionado?');
    if (opcion == true) {
